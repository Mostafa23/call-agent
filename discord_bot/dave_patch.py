...
logger = logging.getLogger("DAVE_Patch")
...
def _patched_decode_packet(self: PacketDecoder, packet: Any) -> Tuple[Any, bytes]:
    ...
    def try_dave_decrypt(data_bytes: bytes) -> bytes:
        ...
        try:
            ...
            # If DAVE is enabled and active on this connection
    ...
    if packet:
        ...
        try:
            ...
            logger.debug(f"[DAVE] Corrupted Opus after decrypt attempt: {e}")
...
def apply_dave_patch():
    """Patches PacketDecoder to decrypt DAVE E2EE encrypted Opus packets before libopus decoding."""
    PacketDecoder._decode_packet = _patched_decode_packet
    logger.info("🛡️ [DAVE Patch] Successfully hooked DAVE E2EE decryption into discord-ext-voice-recv.")
