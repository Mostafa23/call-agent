...
async def check_edge_tts():
    ...
    try:
        import edge_tts
        t0 = time.perf_counter()
        communicate = edge_tts.Communicate("Holding for fact check. Official sources verified.", "en-US-ChristopherNeural")
        audio_chunks = []
        ...
        lat = (time.perf_counter() - t0) * 1000
        print(f"  ✅ Edge-TTS (Neural Voice): Generated {len(total_audio)} audio bytes in {lat:.1f}ms!")
        return True
    except Exception as e:
